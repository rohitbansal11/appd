...
export const getNotification = asyncHandler(async (req, res, next) => {
    ...
    var today_date_check = today_date.getDay() + ":" + today_date.getMonth() + ":" + today_date.getFullYear();
    var compare_date_check = compare_date.getDay() + ":" + compare_date.getMonth() + ":" + compare_date.getFullYear();
    let compare = today_date_check == compare_date_check
    if (compare) {
        SentAllNotification(200, res)
    } else {
        await NotificationCheck.findByIdAndUpdate(id, { verifyDate: today_date })
...
const CheckExpireDate = async (statusCode, res) => {
    ...
    await AllNotification.map(async (w) => {
        ...
        var expire_date_check = compare_date.getDay() + ":" + compare_date.getMonth() + ":" + compare_date.getFullYear();
        let compare = today_date_check == expire_date_check
        console.log(compare)
        if (compare) {
            let dddd = await Notification.findByIdAndDelete(w._id)
            console.log(dddd)
        } else {
            arr.push(w)
        }
